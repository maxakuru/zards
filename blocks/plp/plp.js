...
export async function lookupProducts(config, facets = {}) {
  if (!window.productIndex) {
    ...
    const populateIndex = async (data) => {
      ...
      for (let i = 0; i < topLevelProducts.length; i += 1) {
        ...
        const variants = row.variantSkus
          ? row.variantSkus.split(",").map((e) => skuIndex[e.trim()])
          : [];
        ...
        if (availability.includes("InStock")) {
          ...
          const images = (
            (variants[0] && variants[0].images
              ? variants[0].images
              : row.images) || ""
          ).split(",");

          row.image = new URL(
            images[0] || "missing.png",
            new URL(row.path, window.location.href)
          ).toString();
        }
        products.push(row);
      }
    ...
    window.productIndex = {
      data: json.data.filter((row) => row.sku && !row.sku.includes("test")),
      lookup,
    };
  }
  ...
  if (Array.isArray(config)) {
    ...
    return pathnames
      .map((path) => window.productIndex.lookup[path])
      .filter((e) => e);
  }
...
function createProductButton(product, ph, label, btnClass) {
  ...
  button.querySelector("a").addEventListener("click", async (e) => {
    ...
    // button.querySelector("a").textContent = "Adding...";
    button.querySelector("a").setAttribute("aria-disabled", "true");
...
function createProductCard(product, ph) {
  ...
  const viewDetails = createProductButton(
    product,
    ph,
    "Add to Cart",
    "emphasis"
...
function buildFiltering(block, ph, config) {
  ...
  const highlightResults = (res) => {
    ...
    if (fulltext) {
      res.querySelectorAll("h4").forEach((title) => {
        ...
        if (offset >= 0) {
          title.innerHTML = `${content.substring(
            0,
            offset
  ...
  const displayFacets = (facets, filters) => {
    ...
    addEventListeners(
      ...
      (event) => {
        if (event.currentTarget === event.target)
          block.querySelector(".plp-facets").classList.remove("visible");
      }
    );
    ...
    facetsElement
      .querySelector(".plp-filters-clear")
    ...
    facetKeys.forEach((facetKey) => {
      ...
      div.append(h3);
      const facetValues = Object.keys(facets[facetKey]).sort((a, b) =>
        a.localeCompare(b)
      );
  ...
  const runSearch = async (filterConfig = config) => {
    ...
    console.log(results);
    const sortBy = document.getElementById("plp-sortby")
      ? document.getElementById("plp-sortby").dataset.sort
      : "featured";
